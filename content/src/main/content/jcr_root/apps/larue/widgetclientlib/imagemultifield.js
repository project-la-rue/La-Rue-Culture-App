...
(function () {
    ...
    function addThumbnail(imageField, mName, counter) {
        ...
        function isFileNotFilled(data, counter, fRef) {
            return _.isEmpty(data[mName]) || _.isEmpty(data[mName][counter]) || _.isEmpty(data[mName][counter][fRef])
        }
    }
    ...
    function collectImageFields($form, $fieldSet, counter) {
        ...
        $fields.each(function (j, field) {
            ...
            var prefix = $fieldSet.data("name") + "/" + (counter + 1) + "/",
                ...
                moveSuffix = $widget.inputElement.attr("name") + "/" + new Date().getTime() + SEP_SUFFIX + $fileName.val(),
        ...
        prototype._computeFieldNames = function () {
            ...
            _.each(this.fieldNames, function (value, key) {
                if (value.indexOf("./jcr:") == 0) {
                    ...
                    fieldNames[key] = fileFieldName + removeFirstDot(getStringBeforeAtSign(value)) + SEP_SUFFIX + counter + ".sftmp" + getStringAfterAtSign(value);
                } else {
                    fieldNames[key] = getStringBeforeAtSign(value) + SEP_SUFFIX + counter + getStringAfterAtSign(value);
                }
            });
